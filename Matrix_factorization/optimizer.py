...
import math
from matrix import CONST_Q, CONST_R, CONST_L, SEED, K_LOOP, Subrounte1, finite_consensus, imperfect_consensus
np.random.seed(SEED)
...
def record_results(filename, optimizers, skip):
    for i, opt in enumerate(optimizers):
        with open(filename, mode="a") as csv_file:
            ...
            # file.writerow(opt[5][::skip])
            # file.writerow(["interval_loss"])
            # file.writerow(opt[6][::skip])
            # file.writerow([])


def Algorithm1(w=None, data=None, agents=None, iterations=None, skip=None):
    name = "Algorithm1"
    param_dict = {}
    y_dict = {}
    grad_dict = {}
    ...
    for t in range(iterations):
        ...
        if t % skip == 0:
            print(
                f"Optimizer: {name}, Iteration: {t}, Loss: {math.log10(loss_value[t]):.4f},  Eta: {average_eta[t]:.4f}, Wallclock: {ttt:.4f}")
        if ttt - (last_record_time - start) >= 0.2:
            loss_time.append(math.log10(loss_value[-1]))
            interval_time.append(wallclock[-1])
            last_record_time = c_t

    return name, iters, loss_value, average_eta, wallclock, interval_time, loss_time

def Algorithm2(w=None, data=None, agents=None, iterations=None, skip=None):
    name = "Algorithm2"
    param_dict = {}
    y_dict = {}
    grad_dict = {}
...
def DBBC(w=None, data=None, agents=None, iterations=None, skip=None):
    name = "DGM-BB-C"
    param_dict = {}
    y_dict = {}
    grad_dict = {}
