...
import math
from matrix import subroutine1, finite_consensus, CONST_Q
...
class Base(Optimizer):
    ...
    def cls_collect_params_grads(cls, optimizer: Optimizer, independent: bool = False):
        """
        :param independent: if True, make clone-detach operation, default False
        :param optimizer:
        :return:
        """
        var_s = []
...
class Algorithm3(Base):
    ...
    '''This is a standard gradient tracking algorithm, corresponding to Algorithm 1 in the main text'''
    @classmethod
    ...
    def collect_grad(cls, params: Iterable) -> List[Tensor]:
        """
        collect the gradients of the parameters
        note: no clone-detach operation version
        :param params: Iterable model's parameters
        :return:
        """
        grads = []
        for p in params:
    ...
    def step(self, _type: Literal['x', 'y'], *args, **kwargs) -> None:
        ...
        if _type == 'x':
            ...
            grads: list[Tensor] = kwargs.pop('grads') # get grads[i](x_{i,t},D_{i,t})
            track_grads: dict[int, list[Tensor]] = kwargs.pop('track_grads')  # get grads(x_{i,t+1},D_{i,t})
            prev_track_grads: dict[int, list[Tensor]] = kwargs.pop('prev_track_grads') # get grads(x_{i,t},D_{i,t-1})
            vars_y: dict[int, list[Tensor]] = kwargs.pop('vars_y') # estimate grads(x_{i,t},D_{i,t-1})
            ...
            for group in self.param_groups:
                ...
                for i, p in enumerate(group['params']):
                    ...
                    # * 为了方便，在y的更新中, 可以直接使用$y_{i, t+1}, x_{i, t+1}$参与lr的更新
                    b1 += (p.data - var_x[i + sub]).norm().item() ** 2
                    b2 += (self.y[i + sub] - grads[i + sub]).norm().item() ** 2 # (x_{i,t+1},D_{t}-x_{i,t},D_{t})
