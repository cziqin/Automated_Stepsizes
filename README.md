...
## 🚀 Conclusions
In this repository, we have provided codes that can implement our automated stepsize approach in a fully distributed manner for three machine learning tasks on benchmark datasets. Compared with existing adaptive/nonadpative algorithms, our approach achives a higher convergence and learning accuracy. In addition, to mitigate communication overhead, we have also provided an asynchronous-parallel-update implementation of our approach, which achieves the lowest communication rounds and wallclock time compared with existing couterpart algorithms. To the best of our knowledge, it is the first to completely avoid manual stepsize adjustment in decentralized machine learning tasks without the assistant of any centralized aggregators or information of the global objective function. Given its tuning-free nature and demonstrated effectiveness, our approach is highly applicable to serving as a baseline for evaluating other distributed algorithms in machine learning tasks.
